...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # ((1,1), (1,top), (right, 1), (right, top))  0-unexplore 1-explore
        self.state = [0, 0, 0, 0]
        self.cost = 1
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if (nextx, nexty) == self.corners[3]:
                state5 = 1
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty, state2, state3, state4, state5)
                cost = self.cost
                successors.append((nextState, action, cost))
        self._expanded += 1
        return successors
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    curt = [state[0], state[1]]
    goals = []
    distance = 0
    i = 0
    for corner in state[2:6]:
        if corner == 0:
            goals.append(corners[i])
        i += 1
    while len(goals):
        nearestcorner = findnearestcorner(curt, goals)
        distance += manhattendistence(curt, nearestcorner)
        curt = nearestcorner
        goals.remove(nearestcorner)

    return distance


def manhattendistence(curt, destination):
    xy1 = curt
    xy2 = destination
    return abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])


def findnearestcorner(curt, goals):
    nearestcorner = goals[0]
    shortestdistance = manhattendistence(curt, goals[0])
    for corner in goals[1:]:
        if shortestdistance > manhattendistence(curt, corner):
            nearestcorner = corner
            shortestdistance = manhattendistence(curt, corner)
...
def foodHeuristic(state, problem):
    ...
    return farestfood(goals, position, problem.startingGameState)


def farestfood(nodes, position, GameState):
    distance = 0
    for node in nodes:
        tmpdistance = mazeDistance(position, node, GameState)
...
class ApproximateSearchAgent(Agent):
    ...
    def getAction(self, state):
        ...
        while frontier.isEmpty() == False:
            ...
            if currentList[0] not in exploredSet:
                ...
                if len(successors) == 3:
                    successors = reOrder(successors, food, startPosition)
    ...
    def reOrder(successors, food, startPosition):
        direction = set()
        for successor in successors:
            if successor[1] == "Stop":
                return successors
            direction.add(successor[1])
